...
function getMaxOrderNum($conn) {
    $result = mysqli_query($conn, "SELECT MAX(order_num) as max_order FROM tags");
    $row = mysqli_fetch_assoc($result);
    return $row['max_order'] ?? 0;  // Return 0 if none
}

function renumberTagDescriptions($conn) {
    // Select tags ordered by order_num ascending, then id ascending
    $result = mysqli_query($conn, "SELECT id FROM tags ORDER BY order_num ASC, id ASC");
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        // Update the description and order_num for this tag
        $updateQuery = "UPDATE tags SET description = '{$desc}', order_num = {$count} WHERE id = {$id}";
...
if (isset($_POST['sync_tags'])) {
    ...
    if ($remainingSlots <= 0) {
        ...
        // Get current max order_num once before loop
        $maxOrder = getMaxOrderNum($conn);

        foreach ($featuredTags as $tag) {
            $safeTag = mysqli_real_escape_string($conn, $tag);
            $maxOrder++;  // Increment order number for each new tag

            $insertQuery = "INSERT INTO tags (tag_name, order_num) VALUES ('$safeTag', $maxOrder)";
...
<script>
setInterval(() => {
    fetch('ajax_sync_tags.php')
        .then(res => res.json())
        .then(data => {
            if (data.status === 'success') {
                console.log(`Auto-synced ${data.inserted} tag(s).`);
                // Optional: refresh part of page or show alert
            }
        });
